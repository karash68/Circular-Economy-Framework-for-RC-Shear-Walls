...
#
# MODEL DESCRIPTION:
# - 2D RC shear wall (1m wide × 2m tall × 0.125m original thickness)
# - Shell elements with layered sections (concrete + rebar)
# - Boundary elements (confined) + Interior elements (unconfined), with 30 mm RC jacket on each face
...
# =============================================================================
# LAYERED SHELL SECTIONS - Jacketed multi-layer RC wall cross-sections
# =============================================================================
# Jacketing assumption used here:
# - Added RC jacket on BOTH faces of the original wall
# - Jacket thickness per face = 30 mm
# - Jacket reinforcement = 8 mm bars @ 200 mm in both vertical and horizontal
# - Jacket concrete/material properties kept the same as the original wall
# - Perfect bond is assumed between old wall and new jacket
#
# Original wall thickness  = 125 mm
# Added jacket thickness   = 30 mm + 30 mm
# New total thickness      = 185 mm
#
# Equivalent smeared steel thickness for 8 mm bars @ 200 mm:
#   A_bar = pi*phi^2/4 = 50.27 mm^2
#   t_eq  = A_bar/s = 50.27/200 = 0.2513 mm = 0.0002513 m

jacket_thick = 0.03          # 30 mm per face
jacket_cover = 0.010         # 10 mm outer concrete cover within jacket layer
jacket_core = 0.020          # remaining jacket concrete to interface
jacket_bar_dia = 0.008       # 8 mm
jacket_bar_spacing = 0.20    # 200 mm
jacket_bar_area = math.pi * jacket_bar_dia**2 / 4.0
jacket_rebar_t = jacket_bar_area / jacket_bar_spacing

print(f"Jacket smeared steel thickness per layer = {jacket_rebar_t:.9f} m")

# SECTION 1: JACKETED BOUNDARY ELEMENTS (confined edge zones)
# 14 layers total, thickness = 185 mm
# Jacket is added on both faces; original boundary layering is preserved in the core.
# Outer jacket face: concrete + vertical rebar + horizontal rebar + concrete
# Original wall core: original section 1 layering
# Inner jacket face: concrete + horizontal rebar + vertical rebar + concrete
section('LayeredShell',1,18,
        4,0.010,
        11,jacket_rebar_t,
        10,jacket_rebar_t,
        4,0.020,
...
# SECTION 2: JACKETED INTERIOR ELEMENTS (unconfined center zones)
# 16 layers total, thickness = 185 mm
# Jacket is added on both faces; original interior layering is preserved in the core.
section('LayeredShell',2,16,
        4,0.010,
        11,jacket_rebar_t,
        10,jacket_rebar_t,
        4,0.020,
...
if(AnalysisType=="Pushover"):
	...
	os.makedirs(PushDataDir, exist_ok=True)
	
	# RECORDERS for pushover results
	# Base reactions (sum = base shear)
	recorder('Node', '-file', os.path.join(PushDataDir, 'React.out'), '-closeOnWrite', '-node', 1, 2, 3, 4, 5, '-dof',1, 'reaction')
	# Top displacement (control node)
	recorder('Node', '-file', os.path.join(PushDataDir, 'Disp.out'), '-closeOnWrite', '-node', ControlNode, '-dof',1, 'disp')
	...
	# SAVE AND DISPLAY
	plt.savefig(os.path.join(PushDataDir, 'RCshearwall_PushoverCurve.png'), dpi=1200)
	plt.show()
